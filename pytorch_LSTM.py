...
class LSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, use_bias=True):
        ...
        self.w_ih = nn.Parameter(torch.Tensor(gate_size, input_size)).cuda()
        self.w_hh = nn.Parameter(torch.Tensor(gate_size, hidden_size)).cuda()
        self.b_ih = nn.Parameter(torch.Tensor(gate_size)).cuda()
        self.b_hh = nn.Parameter(torch.Tensor(gate_size)).cuda()
    ...
    def reset_parameters(self):
        ...
        init.orthogonal(self.w_ih.data)
        weight_hh_data = torch.eye(self.hidden_size)
        weight_hh_data = weight_hh_data.repeat(4, 1).cuda()
        self.w_hh.data.set_(weight_hh_data)
        # The bias is just set to zero vectors.
        init.constant(self.b_ih.data, val=0)
        init.constant(self.b_hh.data, val=0)
    ...
    def forward(self, input_, hidden):
        ...
        hx = hx_o.view(-1, self.hidden_size)
        cx = cx_o.view(-1, self.hidden_size)
        
        gates = F.linear(input_, self.w_ih, self.b_ih) + F.linear(hx, self.w_hh, self.b_hh)
        ...
        return hy.view(hx_o.size()), cy.view(cx_o.size())
        #return hy, cy
...
class BNLSTMCell(nn.Module):
    ...
    def forward(self, input_, hx, time):
        ...
        wh = torch.mm(h_0, self.w_hh)
        wi = torch.mm(input_, self.w_ih)
...
class LSTM(nn.Module):
    ...
    @staticmethod
    def _forward_rnn(cell, input_, length, hx, variational_mask=None):
        ...
        for time in range(max_time):
            if isinstance(cell, BNLSTMCell):
                h_next, c_next = cell(input_=input_[time], hidden=hx, time=time)
            else:
                h_next, c_next = cell(input_=input_[time], hidden=hx)
            mask = (time < length).float().unsqueeze(1).expand_as(h_next)
            if variational_mask is not None:
                h_next = h_next*variational_mask
    ...
    def forward(self, variational_mask, input_, hx=None, length=None):
        if self.batch_first:
        ...
        #print(hx[0].size())
        if hx is None:
            hx = Variable(input_.data.new(1, batch_size, self.hidden_size).zero_()).cuda()
            hx = (hx, hx)
        h_n = []
        c_n = []
        layer_output = None
        for layer in range(self.num_layers):
            ...
            layer_output, (layer_h_n, layer_c_n) = LSTM._forward_rnn(
                cell=cell, input_=input_, length=length, hx=hx, variational_mask=variational_mask)
